...
year <- 2019 #currently only possible with 2019
...
codedir <- '~/Dropbox/code_ari/move_comm_analysis/meerkat_vocal_interactions'
...
#directories
#audio.file <- paste(datadir, '/', group, year, '_ALL_CALLS_SYNCHED.csv',sep='')
audio.file <- paste(datadir, '/', 'full_labelfile_conflicts_resolved.csv', sep = '')
gps.file <- paste(datadir ,'/', group , year, '_COORDINATES_', 'all_sessions.RData', sep = '')
...
#parse to simple call types # TO DO! (for now, we miss any hybrid calls!)
calls.all$callTypeSimple <- calls.all$callType 
...
calls.all$isCallerCallType[which(calls.all$callTypeSimple %in% caller.calltypes)] <- T
calls.all$isResponderCallType[which(calls.all$callTypeSimple %in% responder.calltypes)] <- T

#---------------------------RENAME GPS DATA OBJECTS-----------------------
#TODO: GENERALIZE
timeLine <- HM2019_timeLine
allX <- HM2019_allX
allY <- HM2019_allY
indInfo <- HM2019_indInfo
...
for(d in 1:length(dates)){
  ...
  calls.use.date <- calls.date[which((calls.date$isCallerCallType | calls.date$isResponderCallType) & calls.date$focalType == 'F'),]
...
if(self.responses == T | repeat.self.analysis == T){
  callresp.filt <- callresp[which(callresp$caller == callresp$responder),]
} else{
  callresp.filt <- callresp[which(callresp$caller != callresp$responder),]
}

#create a matrix to hold call/response sequences
callresp.seqs <- matrix(data=NA,nrow=nrow(callresp.filt),ncol = length(tseq))

#loop through and get individual call patterns starting at tf
for(i in 1:nrow(callresp.seqs)){
  zero.time <- callresp.filt$t0[i]
  responder <- callresp.filt$responder[i]
  
  #if too close to end, don't use
  if(((zero.time - max.lag) > callresp.filt$start[i]) & ((zero.time + max.lag) < callresp.filt$end[i])){
    
    #get call times of the responder (only of the correct call type, and removing the current call)
    call.times.foc <- calls.use$t0[which(calls.use$isResponderCallType & calls.use$caller == responder)]
...
if(repeat.self.analysis == T){
  ...
  for(i in 1:nrow(callresp.filt)){
    ...
    if(length(inds.in.range.idxs)>0){
      ...
      calls.inds.in.range <- calls.all[which(calls.all$ind %in% inds.in.range & calls.all$isCall==1 & calls.all$focalType=='F'),]
